...
SCREEN_NAME="bore_http_tunnel"
TMP_LOG="/tmp/bore-port-http.log"

# Kill old screen if exists
screen -S "$SCREEN_NAME" -X quit 2>/dev/null

# Start Bore in a detached screen
screen -dmS "$SCREEN_NAME" bash -c "bore local 443 --to bore.pub"
...
echo ""
echo "Service: HTTPS (Secure)"
echo "WARN: TLS Certificate may fail. This is likelly the same as HTTP tunnel but at port 443."
echo "Host: https://ip.orbitsrv.qzz.io:$PORT"
echo ""
echo "[!] Check tunnel status by using 'screen -r $SCREEN_NAME' (Remember to press CTRL A + D to leave logs, CTRL C will kill your tunnel)"
